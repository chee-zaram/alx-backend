#!/usr/bin/env python3
"""
This module contains the class `MRUCache` as a caching system.
"""
...
class MRUCache(BaseCaching):
    ...
    def __init__(self):
        """Initializes MRUCache instances.
        """
        super().__init__()
        self.queue = []
    ...
    def put(self, key: Any, item: Any) -> None:
        """
        `put` uses the MRU algorithm to insert a new item with given key
        into the cache. If the cache is full, it replaces the most recently
        ...
        if len(self.cache_data) >= BaseCaching.MAX_ITEMS:
            ...
            if key not in self.cache_data and mru_key:
                try:
                    del self.cache_data[mru_key]
                    print("DISCARD:", mru_key)
                except KeyError:
                    msg = "Could not insert [{}:{}] as MRU key not in cache"
